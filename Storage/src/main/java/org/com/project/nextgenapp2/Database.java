package org.com.project.nextgenapp2;
...
public class Database {
    public static void createDatabase(){
        ...
        String seedAdmin = "IF NOT EXISTS ( SELECT Username FROM GAME_ITEMS.dbo.Users WHERE Username = 'oezeobi')\n" +
                "BEGIN\nINSERT INTO GAME_ITEMS.dbo.Users(FirstName, Username, Password, ShippingAddress, isAdmin)\n" +
                "VALUES('Ebube', 'oezeobi', 'iamadmin', '1175 Idlewood Ave.', 1)\nEND";
        ...
        String seedInventory = "IF NOT EXISTS " +
                "( SELECT ItemName FROM GAME_ITEMS.dbo.Inventory WHERE ItemName = 'Fortnite Classic Black T-shirt')\n" +
                "BEGIN\n" +
                "    INSERT INTO GAME_ITEMS.dbo.Inventory(ItemName, Description, Price) " +
                "VALUES('Fortnite Classic Black T-shirt', 'A Black T-Shirt Branded By Fortnite', 40.00)\n" +
                "END";
        ...
        String seedShipping1 = "IF NOT EXISTS " +
                "( SELECT ShippingName FROM GAME_ITEMS.dbo.ShippingType WHERE ShippingName = 'Standard Shipping')\n" +
                "BEGIN\n" +
                "    INSERT INTO GAME_ITEMS.dbo.ShippingType (ShippingName, Price) " +
                "VALUES('Standard Shipping', 0.00)\n" +
                "END";

        String seedShipping2 = "IF NOT EXISTS " +
                "( SELECT ShippingName FROM GAME_ITEMS.dbo.ShippingType WHERE ShippingName = '3-Day')\n" +
                "BEGIN\n" +
                "    INSERT INTO GAME_ITEMS.dbo.ShippingType (ShippingName, Price) " +
                "VALUES('3-Day', 12.99)\n" +
                "END";

        String seedShipping3 = "IF NOT EXISTS " +
                "( SELECT ShippingName FROM GAME_ITEMS.dbo.ShippingType WHERE ShippingName = 'Overnight Shipping')\n" +
                "BEGIN\n" +
                "    INSERT INTO GAME_ITEMS.dbo.ShippingType (ShippingName, Price) " +
                "VALUES('Overnight Shipping', 20.99)\n" +
                "END";
        ...
        try {
            ...
            stmt.executeUpdate(seedInventory);
            stmt.executeUpdate(dbShipping);
            stmt.executeUpdate(seedShipping1);
            stmt.executeUpdate(seedShipping2);
            stmt.executeUpdate(seedShipping3);
    ...
    private static void showAllUsers(){
        String allUsers = "SELECT UserID, FirstName, Username, ShippingAddress, isAdmin FROM GAME_ITEMS.dbo.Users" +
                " WHERE NOT isAdmin = 1";
        Connection conn = null;
        String dbURL = "jdbc:sqlserver://obie\\sqlexpress;portNumber=14337;" + "encrypt=true;"
                + "trustServerCertificate=true;"
        ...
        try {
            ...
            while (rs.next()){
                ...
                System.out.println("User ID: "+ id +", First Name: " + firstname + "," +
                        " Username: " +  username + ", Shipping Address: " + shippingAddress + ", isAdmin: " + isAdmin);
            }
        } catch (SQLException ex) {
            ex.printStackTrace();
        } finally {
    ...
    private static void promoteUser(Users newAdmin){
        String userInfo = "IF EXISTS " +
                "( SELECT 1 FROM GAME_ITEMS.dbo.Users WHERE Username = '" + newAdmin.getUserName() + "' )\n" +
                "BEGIN\n" +
                "    SELECT UserID, FirstName, Username, ShippingAddress, isAdmin " +
                "FROM GAME_ITEMS.dbo.Users\n" +
                "    WHERE Username = '" + newAdmin.getUserName() + "' " +
                "END";
        int bit = 0;
        if (newAdmin.getAdmin()){
            bit = 1;
        }
        String promotedUser = "UPDATE GAME_ITEMS.dbo.Users " +
                "SET isAdmin = " + bit + " WHERE UserID in (" + newAdmin.getUniqueID() + ")";
        Connection conn = null;
        String dbURL = "jdbc:sqlserver://obie\\sqlexpress;portNumber=14337;" + "encrypt=true;"
                + "trustServerCertificate=true;"
                + "loginTimeout=30;";
        ...
        try {
            ...
            System.out.println("Connected");
            if(stmt.execute(userInfo)){
                stmt.executeUpdate(promotedUser);
                ResultSet rs = stmt.executeQuery(userInfo);
                while(rs.next()){
                    int id = rs.getInt("UserID");
                    String firstName = rs.getString("FirstName");
        ...
        do{
            ...
            while (!loggedOut){
                ...
                if(choice2 == 1){
                    ...
                    String username = myScan.nextLine();
                    Users promoteUser = retrieveUser(username);
                    promoteUser.setAdmin(true);
